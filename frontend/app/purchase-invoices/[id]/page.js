...
export default function ViewPurchaseInvoicePage() {
  ...
  const [itemMaster, setItemMaster] = useState({}) // Store item master data
  ...
  const logoUrl = typeof window !== 'undefined' ? window.location.origin + (isFourPlastic ? '/logo_four_plastic.png' : '/logo_raj_plastic.png') : '';
  ...
  const emailText = isFourPlastic ? 'fourrplastic@gmali.com' : 'contact@rajplastic.com';

  const companyName = isFourPlastic ? "FOUR (R) PLASTIC" : "RAJ PLASTIC";
  const companyAddress = isFourPlastic 
    ? "90 FEET ROAD, WIRE GALI, KHADI NO.3, ANISH COMPOUND, SAKINAKA, MUMBAI - 400078" 
    : "3 NO. KHADI, SAKINAKA, MUMBAI, MAHARASHTRA - 400072";
  const companyGST = isFourPlastic ? "27ABTPV9432F1ZX" : "27DEFAULTGST";

  const reactToPrintFn = useReactToPrint({ contentRef: printRef, documentTitle: `Purchase_Invoice_${invoiceId || 'loading'}` })
  ...
  const handlePrint = () => {
    ...
    const isSales = false;
    const invoiceTitle = "PURCHASE INVOICE";

    // Set up party details
    let sellerName = invoice.vendor_name || "";
    let sellerAddress = `${invoice.ship_to_address || ''}, ${invoice.ship_to_city || ''}, ${invoice.ship_to_state || ''} ${invoice.ship_to_pincode || ''}`.trim();
    let sellerGST = invoice.ship_to_gst || invoice.vendor_gst || 'URD';
    let sellerState = invoice.ship_to_state || "";

    let buyerName = companyName;
    let buyerAddress = invoice.bill_to_address || companyAddress;
    let buyerGST = invoice.bill_to_gst || companyGST;
    let buyerState = "Maharashtra";

    let consigneeName = companyName;
    let consigneeAddress = invoice.bill_to_address || companyAddress;
    let consigneeGST = invoice.bill_to_gst || companyGST;
    let consigneeState = "Maharashtra";
              ...
              <table class="items-table">
                ...
                <tbody>
                  ...
                  ${isIGST ? `
                    <tr class="tax-row">
                      <td></td>
                      <td class="text-right" style="padding-right: 20px;">IGST ON PURCHASES @ ${igstRate}%</td>
                      <td></td><td></td><td></td>
                  ...
                  ` : `
                    ${(invoice.cgst_total > 0 || invoice.cgst_amount > 0) ? `
                      <tr class="tax-row">
                        <td></td>
                        <td class="text-right" style="padding-right: 20px;">CGST ON PURCHASES @ ${cgstRate}%</td>
                        <td></td><td></td><td></td>
                    ...
                    ${(invoice.sgst_total > 0 || invoice.sgst_amount > 0) ? `
                      <tr class="tax-row">
                        <td></td>
                        <td class="text-right" style="padding-right: 20px;">SGST ON PURCHASES @ ${sgstRate}%</td>
                        <td></td><td></td><td></td>
            ...
            <div class="footer-grid">
              ...
              <div class="footer-right-col">
                ...
                <div class="company-sig-space">
                  <span class="for-company-name">for ${buyerName}</span>
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      await axios.put(`${API_BASE_URL}/purchase-invoices/${invoiceId}/attachment`, {
  ...
  const fetchItemMaster = async () => {
    try {
      ...
      // Create a lookup object for quick access by item_id using compound keys
      const itemLookup = {}
      response.data.forEach(item => {
        const key = item.compound_id || `${item.item_type || (item.type === 'raw_material' || item.type === 'RM' ? 'raw_material' : 'finished_good')}_${item.id}`;
        itemLookup[key] = item
      })
      setItemMaster(itemLookup)
      console.log('Item master loaded:', itemLookup)
    } catch (error) {
  ...
  const fetchInvoice = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/purchase-invoices/${invoiceId}`)
  ...
  const handleEdit = () => {
    router.push(`/purchase-invoices/edit/${invoiceId}`)
  }

  const handleDelete = async () => {
    try {
      await axios.delete(`${API_BASE_URL}/purchase-invoices/${invoiceId}`)
  ...
  // Helper function to get item name from master
  const getItemName = (item) => {
    if (!item) return 'Unknown Item'
    
    // Try to get from item master first using compound key
    const itemType = item.item_type || 'raw_material';
    const key = `${itemType}_${item.item_id}`;
    if (itemMaster[key]) {
      return itemMaster[key].name
    }
    
    // Fallback to what's in the invoice
    return item.item_name || `Item #${item.item_id}`
  }

  // Helper function to get item HSN from master
  const getItemHsn = (item) => {
    ...
    const itemType = item.item_type || 'raw_material';
    const key = `${itemType}_${item.item_id}`;
    if (itemMaster[key] && itemMaster[key].hsn_code) {
      return itemMaster[key].hsn_code
    }
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="relative">
        ...
        {/* Invoice Content */}
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8" ref={printRef}>
          ...
          <div className="relative bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl p-8 print:bg-white print:text-black overflow-hidden">
            {/* Top brand color strip - print only */}
            <div className="absolute top-0 right-0 w-[45%] h-[6px] hidden print:flex z-10">
              <div style={{ backgroundColor: primaryColor }} className="h-full w-[65%]" />
              <div style={{ backgroundColor: secondaryColor }} className="h-full w-[35%]" />
            </div>

            {/* Watermark Logo - print only */}
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none z-0 p-8 hidden print:flex opacity-[0.05]">
              <img src={logoUrl} alt="Watermark" className="w-[280px] h-auto" />
            </div>
            ...
            <div className="flex justify-between items-start mb-8 z-10 relative">
              <div>
                <div className="print:hidden">
                  ...
                  <p className="text-white/60 text-sm">{companyAddress}</p>
                  <p className="text-white/60 text-sm">GST: {companyGST} | PAN: AAAAA0000A</p>
                </div>
                <div className="hidden print:block mt-2">
                  <img src={logoUrl} alt="Logo" className={isFourPlastic ? "w-48 h-auto" : "w-24 h-auto"} />
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8">
              <div>
                <h3 className="text-white/80 text-sm font-semibold mb-2">Bill To (Your Company)</h3>
                <p className="text-white font-medium">FOUR (R) PLASTIC</p>
              ...
              <div>
                <h3 className="text-white/80 text-sm font-semibold mb-2">Ship To (Vendor)</h3>
            ...
            {/* Items Table - FIXED: Now fetches names from item master */}
            <div className="mb-8">
                ...
                <table className="w-full">
                  <thead className="bg-white/5">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left text-xs font-medium text-white/60">Lot/Batch</th>
                      <th className="px-4 py-2 text-right text-xs font-medium text-white/60">Bill Qty</th>
                      <th className="px-4 py-2 text-right text-xs font-medium text-white/60">Recv Qty</th>
                  ...
                  <tbody className="divide-y divide-white/10">
                    {invoice.items?.map((item, index) => {
                      ...
                      const uniqueKey = `item-${item.item_id}-${index}`
                      ...
                      return (
                        <tr key={uniqueKey}>
                          ...
                          <td className="px-4 py-3 text-sm text-white">{item.lot_no || '-'}</td>
                          <td className="px-4 py-3 text-sm text-white text-right">{item.quantity}</td>
                          <td className="px-4 py-3 text-sm text-white text-right">{item.received_quantity !== undefined && item.received_quantity !== null ? item.received_quantity : item.quantity}</td>
            ...
            <div className="mt-8 pt-4 border-t border-slate-200 hidden print:block z-10">
              <div className="flex mb-4 h-[2px]">
                <div style={{ backgroundColor: primaryColor }} className="h-full w-[65%]" />
                <div style={{ backgroundColor: secondaryColor }} className="h-full w-[35%]" />
              </div>
              <div className="flex justify-between items-center text-[8px] font-bold text-slate-800 uppercase">
                <div className="flex items-center gap-2">
                  <div style={{ borderColor: primaryColor }} className="w-4 h-4 rounded-full border flex items-center justify-center">
                    <svg viewBox="0 0 24 24" className="w-2.5 h-2.5" fill="none" stroke={primaryColor} strokeWidth="2.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                  </div>
                  <span className="max-w-[220px] leading-tight text-slate-700">
                    {addressText}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <div style={{ borderColor: primaryColor }} className="w-4 h-4 rounded-full border flex items-center justify-center">
                    <svg viewBox="0 0 24 24" className="w-2.5 h-2.5" fill="none" stroke={primaryColor} strokeWidth="2.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
                  </div>
                  <span className="text-slate-700">{emailText}</span>
                </div>
              </div>
            </div>
          </div>
