...
export default function VendorLedgerPage() {
  ...
  useEffect(() => {
    checkAuth()
    if (vendorId && dateRange.from_date && dateRange.to_date) {
      fetchLedger()
    }
  }, [vendorId, dateRange])
  ...
  // Helper function to format description with items (Qty × Rate)
  const formatDescriptionWithItems = (transaction) => {
    if (!transaction) return '-'
    
    // If it's a purchase transaction with items, show each item with Qty × Rate
    if (transaction.transaction_type === 'PURCHASE' && transaction.items && transaction.items.length > 0) {
      return (
        <div className="space-y-1">
          ...
          {transaction.challan_notes && (
            <div className="text-xs text-green-400 mt-1">
              📝 Note: {transaction.challan_notes}
            </div>
          )}
        </div>
      )
  ...
  const fetchLedger = async () => {
    try {
      ...
      const response = await axios.get(
        `/api/vendor-ledger/${vendorId}?from_date=${dateRange.from_date}&to_date=${dateRange.to_date}&_=${Date.now()}`
      )
      console.log('Ledger data:', response.data)
      
      let transactions = response.data.transactions || []
      
      // Fetch delivery challan notes for purchase transactions
      console.log('Fetching delivery challan notes for purchase transactions...')
      for (let i = 0; i < transactions.length; i++) {
        const trans = transactions[i]
        
        // If it's a purchase transaction
        if (trans.transaction_type === 'PURCHASE' && trans.reference_no) {
          try {
            // Step 1: Get the purchase invoice details
            const purchaseInvoiceResponse = await axios.get(`/api/purchase-invoices/by-number/${trans.reference_no}`)
            
            if (purchaseInvoiceResponse.data) {
              const purchaseInvoice = purchaseInvoiceResponse.data
              const challanNo = purchaseInvoice.challan_no
              
              console.log(`Purchase ${trans.reference_no} has challan: ${challanNo}`)
              
              // Step 2: If there's a challan number, fetch the delivery challan
              if (challanNo) {
                try {
                  const challanResponse = await axios.get(`/api/delivery-challans/by-number/${challanNo}`)
                  if (challanResponse.data) {
                    const narration = challanResponse.data.notes || challanResponse.data.narration || ''
                    
                    if (narration) {
                      transactions[i].challan_notes = narration
                      transactions[i].challan_no = challanNo
                      console.log(`✅ Found narration for ${trans.reference_no}: "${narration}" (from challan ${challanNo})`)
                    }
                  }
                } catch (challanError) {
                  console.log(`❌ Could not fetch delivery challan ${challanNo}`)
                }
              }
            }
          } catch (error) {
            console.log(`❌ Could not fetch purchase invoice for ${trans.reference_no}`)
          }
        }
      }
      
      // Update the ledger with enhanced transactions
  ...
  const calculateStats = () => {
    ...
    setStats({
      totalPurchases: ledger.summary?.total_purchases || 0,
      totalPayments: ledger.summary?.total_payments || 0,
      totalInvoices: ledger.transactions?.filter(t => t.transaction_type === 'PURCHASE').length || 0,
      averageInvoice: ledger.summary?.total_purchases > 0 && ledger.summary?.total_invoices > 0 
        ? ledger.summary.total_purchases / ledger.summary.total_invoices 
        : 0,
      outstanding: ledger.summary?.closing_balance || ledger.vendor?.current_balance || 0,
      lastTransactionDate: ledger.transactions?.length > 0 
  ...
  const handleWhatsAppReminder = () => {
    if (!ledger?.vendor) return;
    const phone = ledger.vendor.phone || ledger.vendor.mobile;
    if (!phone) {
    ...
    const balance = ledger.vendor.current_balance;
    const vendorName = ledger.vendor.name;
    const date = new Date().toLocaleDateString('en-IN');
    
    let text = `Dear ${vendorName},\n\nGreetings from FOUR (R) PLASTIC.\n\nThis is regarding your account balance of *${formatCurrency(Math.abs(balance))}* as of ${date}.\n\nThank you for your partnership!`;
  ...
  const handlePrint = () => {
    ...
    const content = `
      ...
      <html>
      <head>
        <title>Vendor Ledger - ${ledger?.vendor?.name}</title>
        <style>
          ...
          .challan-note { color: #059669; font-size: 10px; margin-top: 6px; padding: 4px 8px; background: #ecfdf5; border-radius: 4px; display: inline-block; border: 1px solid #a7f3d0; }
          .ref-text { font-family: monospace; color: #374151; font-size: 11px; }
          .dc-text { color: #db2777; font-size: 10px; margin-top: 2px; }
        </style>
      </head>
      <body>
        ...
        <div class="info-grid">
          <div class="party-info">
            ...
            <p style="font-weight: 600; font-size: 14px;">${ledger?.vendor?.name}</p>
            <p style="font-family: monospace; color: #6b7280;">${ledger?.vendor?.code}</p>
          </div>
        ...
        <div class="stats-grid">
          <div class="stat-card">
            ...
            <div class="stat-value">${formatCurrency(ledger?.vendor?.opening_balance || 0)}</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Total Purchases</div>
            <div class="stat-value">${formatCurrency(ledger?.summary?.total_purchases || 0)}</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Total Payments</div>
            <div class="stat-value">${formatCurrency(ledger?.summary?.total_payments || 0)}</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Closing Balance</div>
            <div class="stat-value ${ledger?.summary?.closing_balance > 0 ? 'debit' : 'credit'}">${formatCurrency(ledger?.summary?.closing_balance || 0)}</div>
          </div>
        </div>
        ...
        <table>
          ...
          <tbody>
            ${filteredTransactions.map(trans => {
              const isPurchase = trans.transaction_type === 'PURCHASE' && trans.items && trans.items.length > 0;
              let itemsHtml = isPurchase ? trans.items.map(i => `<div class="item-line" style="margin:2px 0;">${i.item_name}</div>`).join('') : (trans.description || '-');
              if (trans.challan_notes) {
                  itemsHtml += '<div class="challan-note">&#128221; ' + trans.challan_notes + '</div>'
              }
              const qtyHtml = isPurchase ? trans.items.map(i => `<div class="item-line" style="margin:2px 0; color:#374151;">${i.quantity} ${i.unit||''}</div>`).join('') : '-';
              const rateHtml = isPurchase ? trans.items.map(i => `<div class="item-line" style="margin:2px 0; color:#374151;">${formatCurrency(i.rate)}</div>`).join('') : '-';
              
              const badgeClass = trans.transaction_type === 'PURCHASE' ? 'purchase' : trans.transaction_type === 'PAYMENT' ? 'payment' : '';
              
              return '<tr>' +
                '<td style="white-space: nowrap; vertical-align: top;">' + formatDate(trans.transaction_date) + '</td>' +
                '<td style="vertical-align: top;"><span class="type-badge ' + badgeClass + '">' + trans.transaction_type + '</span></td>' +
                '<td style="vertical-align: top;">' +
                  '<div class="ref-text">' + (trans.reference_no || '-') + '</div>' +
                  (trans.challan_no ? '<div class="dc-text">&#128230; DC: ' + trans.challan_no + '</div>' : '') +
                '</td>' +
                '<td style="vertical-align: top;">' + itemsHtml + '</td>' +
                '<td class="text-right" style="vertical-align: top;">' + qtyHtml + '</td>' +
                '<td class="text-right" style="vertical-align: top;">' + rateHtml + '</td>' +
                '<td class="text-right debit" style="vertical-align: top;">' + (trans.debit != null && trans.debit !== 0 ? formatCurrency(trans.debit) : '-') + '</td>' +
                '<td class="text-right credit" style="vertical-align: top;">' + (trans.credit != null && trans.credit !== 0 ? formatCurrency(trans.credit) : '-') + '</td>' +
                '<td class="text-right balance-val" style="vertical-align: top;">' + formatCurrency(trans.balance) + '</td>' +
              '</tr>'
            }).join('')}
          </tbody>
        </table>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/70 mt-4">Loading vendor ledger...</p>
        </div>
      </div>
    )
  }
  ...
  if (!ledger) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-white text-xl">Vendor not found</div>
      </div>
    )
  }
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-white/60 text-sm">{ledger.vendor?.name} ({ledger.vendor?.code})</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => router.push(`/payments/new?type=payment&party_id=${vendorId}`)}
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="bg-gradient-to-br from-blue-500/10 to-cyan-500/10 backdrop-blur-xl border border-white/20 rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white font-bold text-lg">{formatCurrency(ledger?.summary?.total_purchases || 0)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 backdrop-blur-xl border border-white/20 rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white font-bold text-lg">{formatCurrency(ledger?.summary?.total_payments || 0)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 backdrop-blur-xl border border-white/20 rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white font-bold text-lg">{ledger.transactions?.filter(t => t.transaction_type === 'PURCHASE').length || 0}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-red-500/10 to-orange-500/10 backdrop-blur-xl border border-white/20 rounded-xl p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-red-400 font-bold text-lg">{formatCurrency(ledger?.summary?.closing_balance || 0)}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl p-4">
              ...
              <p className="text-white font-bold text-lg">{formatCurrency(ledger.vendor?.opening_balance || 0)}</p>
            </div>
            <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl p-4">
              <p className="text-white/60 text-xs">Current Balance</p>
              <p className={`font-bold text-lg ${(ledger.summary?.closing_balance || 0) > 0 ? 'text-red-400' : 'text-green-400'}`}>
                {formatCurrency(ledger.summary?.closing_balance || 0)}
              </p>
            </div>
            <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl p-4">
              <p className="text-white/60 text-xs">Average Invoice</p>
              <p className="text-white font-bold text-lg">
                {formatCurrency(ledger.summary?.total_purchases > 0 && ledger.summary?.total_invoices > 0 
                  ? ledger.summary.total_purchases / ledger.summary.total_invoices 
                  : 0)}
              </p>
            </div>
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-3 mb-4">
            <div className="flex items-center justify-between">
              ...
              {ledger.summary?.lastTransactionDate && (
                <p className="text-white/60 text-sm">
                  Last Transaction: <span className="text-white">{formatDate(ledger.summary.lastTransactionDate)}</span>
                </p>
              )}
            </div>
          </div>
          ...
          <div className="mb-3 flex justify-between items-center">
            ...
            <p className="text-white/60 text-sm">
              Running Balance: <span className="text-white font-semibold">{formatCurrency(filteredTransactions[filteredTransactions.length - 1]?.balance || ledger.vendor?.opening_balance || 0)}</span>
            </p>
          </div>
              ...
              <table className="w-full">
                <thead className="bg-white/5">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-white/60">Description / Items</th>
                    ...
                    <th className="px-4 py-3 text-right text-xs font-medium text-white/60">Debit (Purchase)</th>
                  ...
                  {filteredTransactions.length === 0 ? (
                    ...
                    filteredTransactions.map((trans, index) => (
                      <tr key={index} className="hover:bg-white/5">
                        ...
                        <td className="px-4 py-3 text-sm align-top">
                          <div>
                            <span className="text-white font-mono">{trans.reference_no || '-'}</span>
                            {trans.challan_no && (
                              <span className="block text-xs text-pink-400 mt-0.5">
                                📦 Challan: {trans.challan_no}
                              </span>
                            )}
                          </div>
                        </td>
                        ...
                        {trans.transaction_type === 'PURCHASE' && trans.items && trans.items.length > 0 ? (
                          <>
                            <td className="px-4 py-3 text-sm align-top">
                              <div className="space-y-1">
                                ...
                                {trans.challan_notes && (
                                  <div className="text-xs text-green-400 mt-1">📝 Note: {trans.challan_notes}</div>
                                )}
                              </div>
                            </td>
                          ...
                          <>
                            <td className="px-4 py-3 text-sm align-top text-white/70">
                                {trans.description || '-'}
                                {trans.challan_notes && (
                                  <div className="text-xs text-green-400 mt-1">📝 Note: {trans.challan_notes}</div>
                                )}
          ...
          {filteredTransactions.length > 0 && (
            <div className="mt-4 bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg p-3">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <p className="text-white/50 text-xs">Total Purchases (Debit)</p>
