...
# Database path - use manufacturing database
PROJECT_ROOT = Path(__file__).resolve().parent.parent
DB_PATH = os.path.join(PROJECT_ROOT, 'database', 'manufacturing.db')

# Your complete schema (I'll include it in the script)
schema_sql = """
-- =====================================================
...
-- PRODUCTION ENTRIES (Day-wise production) - Fixed version
...
-- PRODUCTION RAW MATERIAL CONSUMPTION - Fixed version
...
-- Sample Production Entries - Fixed to use operator_name instead of operator_id
INSERT OR IGNORE INTO production_entries (production_no, production_date, shift, finished_good_id, quantity_produced, batch_no, machine_no, operator_name, good_quantity, rejected_quantity, status) VALUES 
('PROD2403001', '2024-03-01', 'Morning', 1, 50, 'B240301-01', 'MACH-01', 'John Doe', 48, 2, 'completed'),
('PROD2403002', '2024-03-01', 'Evening', 1, 45, 'B240301-02', 'MACH-01', 'John Doe', 44, 1, 'completed'),
('PROD2403003', '2024-03-02', 'Morning', 2, 25, 'B240302-01', 'MACH-02', 'Jane Smith', 25, 0, 'completed'),
('PROD2403004', '2024-03-02', 'Evening', 3, 200, 'B240302-02', 'MACH-03', 'Bob Wilson', 195, 5, 'verified');

-- Sample Production Consumption - Fixed to remove rate and total_cost
...
def reset_database():
    ...
    print("="*60)
    print("FOUR® PLASTIC - Database Reset")
    print("="*60)
    ...
    try:
        ...
        # Connect to database (this will create new file)
        ...
        if cursor.fetchone():
            ...
            print(f"✅ vendors table created with {vendor_count} records")
            
            # Show vendors
            cursor.execute("SELECT id, code, name, current_balance FROM vendors")
            vendors = cursor.fetchall()
            for v in vendors:
                print(f"   - {v[0]}: {v[1]} - {v[2]} (Balance: {v[3]})")
        else:
            print("❌ vendors table NOT created!")
        
        # Verify other tables
        tables_to_check = ['customers', 'banks', 'raw_materials', 'finished_goods']
        ...
        print("\n🔍 You can now test:")
        print("   http://localhost:5000/api/vendors/")
        print("   http://localhost:5000/api/vendor-ledger/1")
